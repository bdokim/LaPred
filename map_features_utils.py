...
_MANHATTAN_THRESHOLD = 5.0
_DFS_THRESHOLD_FRONT_SCALE = 50.
_DFS_THRESHOLD_BACK_SCALE = 20.
_MAX_SEARCH_RADIUS_CENTERLINES = 20.
...
class MapFeaturesUtils:
    ...
    def get_candidate_centerlines_for_trajectory(
        ...
        dfs_threshold_front = 60
        dfs_threshold_back = 20
        ...
        for lane in curr_lane_candidates:
          ...
          for past_lane_seq in candidates_past:
            for future_lane_seq in candidates_future:
              assert (past_lane_seq[-1] == future_lane_seq[0]),\
        ...
        if mode == "test":
          ...
          #     [obs_pred_lanes[i]], city_name)[0] for i in range(len(obs_pred_lanes))]

          # direction check
          candidate_centerlines = []
          sp = xy[0:1,:]
          ep = xy[-2:-1,:]
          for i in range(len(obs_pred_lanes)) :
            centerline = avm.get_cl_from_lane_seq([obs_pred_lanes[i]], city_name)[0]

            diff = centerline - sp
            smi = np.argmin(np.square(diff).sum(-1))

            diff = centerline - ep
            emi = np.argmin(np.square(diff).sum(-1))
    ...
    def compute_map_features(
        ...
        agent_xy = agent_track[:, \
            [raw_data_format["X"],raw_data_format["Y"]]].astype("float")
        ...
        if mode == "test":
          ...
          candidate_centerlines = self.get_candidate_centerlines_for_trajectory(
              agent_xy_obs, city_name, avm, viz=False,
              max_search_radius=self._MAX_SEARCH_RADIUS_CENTERLINES,
              seq_len=seq_len, max_candidates=self._MAX_CENTERLINE_CANDIDATES_TEST)
